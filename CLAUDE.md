...
- You are committed to creating the best paper possible and that requires a deep understanding of SCD and its applications as well as the production of stylish, high-quality figures.
...
python -m experiments.run configs/mog_2d_4.yaml   # run one experiment
python -m experiments.run configs/mog_2d_4.yaml --debug  # no JIT
...
|----------|----------------|
| `docs/ARCHITECTURE.md` | Directory layout, layer diagram, dependency rules |
| `docs/ALGORITHMS.md` | ETD variants + baselines: math, pseudocode, naming |
| `docs/API.md` | Sampler interface (`init`/`step`/`extract`), types, configs |
| `docs/DEVELOPMENT.md` | JAX patterns, numerical hygiene, coding style |
| `docs/EXPERIMENTS.md` | Benchmark protocol: targets, metrics, figures |
| `docs/SCHEMA.md` | YAML config format with examples |
| `docs/STYLE.md` | Plot aesthetics: colors, contours, layout |
| `docs/PHASES.md` | ~~Phases for the initial implementation of benchmarks.~~ **All Completed.** |
| `docs/PITCHING.md` | Details on our pitch classification benchmark. |

## Build Order

Implement in the order described in `docs/PHASES.md`. Each phase should be testable before moving on.
...
**Clear notebooks, do not commit heavy media objects.** When commiting Jupyter notebooks, always clear them before commiting them. Do not commit heavy media objects, e.g., images, videos, etc.

**Only run relevant tests.** You should only run the full suit after major changes / refactors. The full test suite takes quite a while to run since it, e.g., runs NUTS on various problems. Identify and only run the most relevant tests.
...
| `config-schema` | Designing config schemas: frozen dataclasses for JIT, Pydantic for YAML validation, Hydra for CLI sweeps. Two-layer config pattern, sweep expansion, translation layers. |
